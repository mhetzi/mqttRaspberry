...
class Config:
    ...
    def get_parity(self):
        parity_str = self._config["parity"]
        try:
            return eval(parity_str)
        except Exception as e:
            g_log.error(f"Kann von {parity_str} in {self._config} parity nicht herausfinden! Exception: {e}")
    ...
    def get_export_format(self):
        return self._config.get("export_format", None)
...
class Obis:
    ...
    VoltageL1 = to_bytes("1.0.32.7.0.255")
...
def setup(g_cfg, custom_logger=None):
    #
    # Script Start
    #
    global g_log
    ...
    return g_log, g_ser, g_supplier

def mainLoop(g_cfg: Config, g_log: logging.Logger, g_ser: serial.Serial, g_supplier: Union[SupplierEVN, SupplierTINETZ], data_callback=None):
...
if __name__ == "__main__":
    ...
    if not g_cfg.load():
        ...
        sys.exit(10)
    g_log, g_ser, g_supplier = setup(g_cfg=g_cfg)
    mainLoop(g_cfg=g_cfg, g_log=g_log, g_ser=g_ser, g_supplier=g_supplier)