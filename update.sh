...
then
    ...
    if [[ $hasVenvInstalled -eq 1 ]]; then
        ...
        if [[ $REPLY =~ ^[YyJj]$ ]]
        then
            sudo bash -c "sudo apt update; sudo apt install python3-pip python3-dev libatlas-base-dev libjpeg9-dev libfreetype6-dev -y; python3 -m pip install virtualenv"
        fi
    fi
    echo "Erselle VENV"
